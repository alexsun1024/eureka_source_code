...
public class EurekaBootStrap implements ServletContextListener {
    ...
    private static final String ARCHAIUS_DEPLOYMENT_ENVIRONMENT = "archaius.deployment.environment";//archaius是Netflix公司开源项目之一，基于java的配置管理类库，主要用于多配置存储的动态获取

    private static final String EUREKA_ENVIRONMENT = "eureka.environment";//配置运行环境
    ...
    private static final String EUREKA_DATACENTER = "eureka.datacenter";//配置数据中心运行环境名称，AWS云环境中，传递java命令行属性-Deureka.datacenter = cloud，以便Eureka客户端/服务器知道初始化特定于AWS云的信息
    ...
    protected void initEurekaServerContext() throws Exception {
        ...
        ServerCodecs serverCodecs = new DefaultServerCodecs(eurekaServerConfig);//服务器编解码器
        ...
        if (isAws(applicationInfoManager.getInfo())) {
            ...
            registry = new PeerAwareInstanceRegistryImpl(  //Aware感知     感知eureka server集群的服务实例注册表
                    eurekaServerConfig,//服务器配置
        ...
        PeerEurekaNodes peerEurekaNodes = getPeerEurekaNodes(    //peer节点处理
                registry,
                eurekaServerConfig,
                eurekaClient.getEurekaClientConfig(),
                serverCodecs,
        ...
        // Copy registry from neighboring eureka node   从相邻的eureka server节点复制注册表，如果拷贝失败，就找下一个
        int registryCount = registry.syncUp();
        registry.openForTraffic(applicationInfoManager, registryCount);//

        // Register all monitoring statistics.  注册所有监视统计信息。
    ...
    /**
     * Handles Eureka cleanup, including shutting down all monitors and yielding all EIPs.    处理Eureka清理，包括关闭所有监视器并产生所有EIP。
     *
    ...
    /**
     * Server context shutdown hook. Override for custom logic        服务器上下文关闭挂钩。 覆盖自定义逻辑
     */

...
public class ResponseCacheImpl implements ResponseCache {
    ...
    ResponseCacheImpl(EurekaServerConfig serverConfig, ServerCodecs serverCodecs, AbstractInstanceRegistry registry) {
        ...
        this.readWriteCacheMap =
                CacheBuilder.newBuilder().initialCapacity(serverConfig.getInitialCapacityOfResponseCache())
                        ...
                        .build(new CacheLoader<Key, Value>() {//从eureka的server注册表中读取
                            @Override
            ...
            public void run() {
                ...
                for (Key key : readOnlyCacheMap.keySet()) {
                    ...
                    try {
                        ...
                        if (cacheValue != currentCacheValue) {//缓存内容不相等
    ...
    public void invalidate(Key... keys) {
        for (Key key : keys) {
            ...
            readWriteCacheMap.invalidate(key);//invalidate无效
    ...
    /**
     * Get the payload in both compressed and uncompressed form.   缓存核心逻辑
     */
    @VisibleForTesting
    Value getValue(final Key key, boolean useReadOnlyCache) {
        ...
        try {
            if (useReadOnlyCache) {
                final Value currentPayload = readOnlyCacheMap.get(key);//只读
                if (currentPayload != null) {
                    payload = currentPayload;
                } else {
                    payload = readWriteCacheMap.get(key);//读写

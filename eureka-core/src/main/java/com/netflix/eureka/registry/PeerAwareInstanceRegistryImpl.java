 ...
 * </p>
 *
 * 这个eureka server启动的时候，会尝试从其他的eureka server上过去抓取注册表的信息，如果抓取失败了，那么就不会让其他的服务实例来自己这里进行服务发现，获取自己的注册表的信息
 *
 * <p>
 ...
 *
 *
 * 处理所有操作到AbstractInstanceRegistry到对等Eureka节点的复制，以使它们保持同步。
 * 复制的主要操作是寄存器，续订，取消，到期和状态更改
 * 当eureka服务器启动时，它将尝试从对等eureka节点获取所有注册表信息。如果由于某种原因该操作失败，则服务器不允许用户在EurekaServerConfig.getWaitTimeInMsWhenSyncEmpty（）中指定的时间段内获取注册表信息。 。
 * 关于续订的重要注意事项：如果续订在EurekaServerConfig.getRenewalThresholdUpdateIntervalMs（）的期间内下降超过EurekaServerConfig.getRenewalPercentThreshold（）中指定的指定阈值，则eureka认为这是一种危险并停止实例过期。
 *  自我保护机制
...
public class PeerAwareInstanceRegistryImpl extends AbstractInstanceRegistry implements PeerAwareInstanceRegistry {
    ...
    public void openForTraffic(ApplicationInfoManager applicationInfoManager, int count) {
        // Renewals happen every 30 seconds and for a minute it should be a factor of 2.  续订每30秒发生一分钟，应为2倍。
    ...
    public void register(final InstanceInfo info, final boolean isReplication) {
        ...
        replicateToPeers(Action.Register, info.getAppName(), info.getId(), info, null, isReplication);//同步注册信息到其他server
    }

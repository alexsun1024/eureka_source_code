...
public abstract class AbstractInstanceRegistry implements InstanceRegistry {
    ...
    public void register(InstanceInfo registrant, int leaseDuration, boolean isReplication) {
        try {
            ...
            if (gMap == null) {//如果服务第一次注册，就会创建一个新的map，registry就是注册表
            ...
            if (existingLease != null && (existingLease.getHolder() != null)) {
                ...
                if (existingLastDirtyTimestamp > registrationLastDirtyTimestamp) {
                    logger.warn("There is an existing lease and the existing lease's dirty timestamp {} is greater" +
                            " than the one that is being registered {}", existingLastDirtyTimestamp, registrationLastDirtyTimestamp);//有一个现有租约，而现有租约的脏时间戳{}大于正在注册的租约{}
                    logger.warn("Using the existing instanceInfo instead of the new instanceInfo as the registrant");//使用现有的instanceInfo代替新的instanceInfo作为注册者
            ...
            // This is where the initial state transfer of overridden status happens 这是发生覆盖状态的初始状态转移的地方
            ...
            // If the lease is registered with UP status, set lease service up timestamp  如果租约已注册为UP状态，请设置租约服务启动时间戳记
            if (InstanceStatus.UP.equals(registrant.getStatus())) {
                lease.serviceUp();
            }
            registrant.setActionType(ActionType.ADDED);
            recentlyChangedQueue.add(new RecentlyChangedItem(lease));//最近注册改变队列
            registrant.setLastUpdatedTimestamp();
            invalidateCache(registrant.getAppName(), registrant.getVIPAddress(), registrant.getSecureVipAddress());//初始化缓存
    ...
    public boolean renew(String appName, String id, boolean isReplication) {
        ...
        if (leaseToRenew == null) {
            ...
            renewsLastMin.increment();//上一分钟心跳次数
            leaseToRenew.renew();
            return true;
        }
    }
    ...
    public void evict(long additionalLeaseMs) {
        ...
        if (!isLeaseExpirationEnabled()) {//是否开启自我保护机制
